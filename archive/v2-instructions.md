...
- **V1 is the regression baseline.** After every shared-module extraction step, confirm the personal page (`index.html`) builds and behaves **byte-for-byte identically** to before. If a refactor changes V1 output, the refactor is wrong — fix it before proceeding.
  ...
  - ✅ **W2 also cleared this gate.** Backend parameterization touched the two serverless functions and the two frontend fetch modules. V1 was held byte-identical by giving the fetch modules **back-compat overloads** (a string `location` arg, or no `options` arg, reproduces the old request exactly), so the three V1 call sites are textually unchanged. Verified by: the unchanged call sites, a green `tsc`/`vite build`, and the V1 backend branches (`?location=` / no-param) being logically untouched — the new `?lat=&lon=` handling is added as branches *above* them.
