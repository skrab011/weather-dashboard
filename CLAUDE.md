...
- Chart: NWS + CAIC + ECMWF (Open-Meteo) on shared axes with a shaded model-disagreement band and a Temp/Wind/Precip/Snow variable toggle. Temp & wind draw all three sources; precip & snow draw amounts (inches) from CAIC + ECMWF only (NWS gives precip probability, not amount). Elevation labeled per series in temp mode. NWS elevation: ~9,035 ft; CAIC elevation: 9,219 ft; ECMWF elevation from the live Open-Meteo grid cell. (ECMWF line + band + toggle added 2026-06-22 — Tracks A/B/C.)
...
- 🏁 **Epic COMPLETE** — D + A + B + C all live on V1 and V2.
