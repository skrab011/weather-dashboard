...
- **Scheduling:** No cron (requires Vercel Pro). Consensus brief is served from the Blob cache while it is under **30 minutes old** (`BRIEF_MAX_AGE_MS` in `api/brief.ts`); the first page load after that age triggers a fresh AI call and re-caches. Worst-case staleness ≈ 40 min (30-min blob age + 10-min CDN `s-maxage`). The age check was added 2026-07-17 — before that, the cached blob was served unconditionally and the brief only updated on manual ↻ Refresh (see build-log "Brief auto-refresh fix")
- **Service worker:** Cache version `weather-v4`. Precaches `/`, `/shared`, `/manifest.json`, `/manifest-shared.json`, `/icons/icon-192.png`. Skips all `/api/` routes so serverless functions are never intercepted. Page navigations (the HTML itself) are fetched **network-first** with the cache as offline fallback (added 2026-07-18 — the old serve-cache-first strategy caused a blank page on the first load/refresh after every deployment, because the stale HTML referenced hashed JS bundles Vercel had already deleted; see build-log "Service worker blank-page fix"). Static assets keep stale-while-revalidate. All cache writes are wrapped in `event.waitUntil()` so the browser can't terminate the SW before the write lands. Both `src/main.ts` (V1) and `src/shared-main.ts` (V2) register the SW and listen for `updatefound` — when a new SW activates after a Vercel deployment, the page auto-reloads so users always get fresh content without a manual refresh. A `hadController` guard ensures this only fires on updates, not on a user's very first visit.
...
| `ELEVENLABS_API_KEY` | `api/radio.ts` (Radio Forecast TTS) |

(`OPENAI_API_KEY` is no longer used by any code — it stays in Vercel as a rollback parachute until ~2026-07-20, then gets deleted. See `elevenlabs-migration-plan.md` §6.)
...
5. **ElevenLabs key / billing for the radio button** — if `ELEVENLABS_API_KEY` lapses or billing fails, the 🎙 Radio button shows "Unavailable" but the brief card is otherwise unaffected. Fix is at https://elevenlabs.io/app/settings/billing — nothing in the app needs changing. Radio also requires a *cached* brief in Blob, so a lapsed `BLOB_READ_WRITE_TOKEN` (fragility #3) breaks it too; fixing the token fixes both. One more radio-specific dependency: the custom voice (`Gw0nY3v7mRqp8whsS8cs`) lives in the owner's ElevenLabs account under **My Voices** — deleting it there breaks the button; fix is crafting/picking a new voice and updating the `TTS_VOICE_ID` constant in `api/radio.ts`.
...
## Now-card wind from HRRR/GFS (2026-07-17)

The **Wind row on the Now card** (V1 and V2) shows current wind from **NOAA HRRR** (3 km rapid-refresh model, updated hourly) instead of the NWS hourly forecast — owner finds the models more accurate. Served by Open-Meteo (already used for the chart), `current=wind_speed_10m,wind_direction_10m` with `models=gfs_hrrr`; **GFS (`gfs_seamless`) is the automatic fallback** (HRRR covers the lower 48 only), and the **NWS hourly wind is the final fallback** if all Open-Meteo calls fail — the row never goes blank and looks identical in every case (no source label, uncluttered-UI priority). Fetch: `fetchCurrentWindResult()` in `src/shared/openmeteo.ts`; state: `currentWind: SourceResult<CurrentWind>` on `LocationWeather`; render: `renderConditions`'s `windResult` param (degrees → 16-point cardinal via `degToCardinal()` in `src/shared/cards.ts`). Hourly strip and 7-day wind columns still use NWS — owner scoped the model wind to the Now card only. To see which source answered, check DevTools → Network for the `models=gfs_hrrr` request.

## Radio Forecast (live on `main`, 2026-07-12)

A **"🎙 Radio" button on the V1 Consensus Brief card** that reads the current brief aloud in the style of a radio weather announcer, via **ElevenLabs** text-to-speech (model `eleven_turbo_v2_5`, the owner's custom Voice Design voice `Gw0nY3v7mRqp8whsS8cs`, ~$0.05 per 1,000 characters ≈ 4¢ per generation). Originally built 2026-07-12 on OpenAI (`gpt-4o-mini-tts`, voice `ash`) per `archive/radio-forecast-plan.md`; **migrated to ElevenLabs 2026-07-13** per **`elevenlabs-migration-plan.md`** — the radio-host delivery and pacing are designed into the custom voice itself, so no `instructions`/`voice_settings` are sent and the OpenAI-era 1.1x client-side playback speed-up was removed (plays at normal speed). The time-of-day greeting ("Good morning/afternoon/evening", Colorado time) is still prepended server-side to the spoken text, and the MP3 cache hash covers the model ID + greeting + brief so a clip regenerates when any of them changes. As-built details in `build-log.md` → "Radio Forecast" and "ElevenLabs migration".
